...
void
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)//set ipv4 and initialize
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
...
void RoutingProtocol::DoDispose ()//close and kill the stream
{
  m_ipv4 = 0;
  m_hnaRoutingTable = 0;
  m_routingTableAssociation = 0;

  if (m_recvSocket)//close the socket
    {
      m_recvSocket->Close ();
      m_recvSocket = 0;
    }
  ...
  for (std::map< Ptr<Socket>, Ipv4InterfaceAddress >::iterator iter = m_sendSockets.begin ();
    ...
    {
      iter->first->Close ();
    }//close the chain of the sendsocket
  m_sendSockets.clear ();
  m_table.clear ();

  Ipv4RoutingProtocol::DoDispose ();//close ipv4
}
...
RoutingProtocol::RecvOlsr (Ptr<Socket> socket)
{
  ...
  //information of receivedPacket and initialize
  ...
  //information of interface(define a node and the device in the node)
  ...
  //get the ip address of the receiver using the method GetAddress() & GetLocal()
  ...
  for (MessageList::const_iterator messageIter = messages.begin ();
       messageIter != messages.end (); messageIter++)
    {
      ...
      // drop the message using the method RemoveAtStart
...
//         }
      ...
      if (duplicated == NULL) //this message are not processed before
        {
      ...
      if (onlyOne)
        {
          ...
          mprSet.insert (twoHopNeigh->neighborMainAddr);//select the only one

          // take note of all the 2-hop neighbors reachable by the newly selected MPR
